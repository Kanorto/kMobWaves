...
- **Зависимости**: 
  - MythicMobs 5.6.1+ (обязательно)
  - ProtocolLib 5.0+ (опционально, для подсветки мобов только для администратора)
...
highlight:
  ...
  # Примечание: В режиме ADMIN с установленным ProtocolLib подсветка видна только администратору
  #             Без ProtocolLib подсветка видна всем игрокам
```
...
| `highlight` | Подсвечивает оставшихся мобов волны на 10 секунд. С ProtocolLib в режиме ADMIN видно только админу, иначе всем | `kmobwaves.highlight` или `kmobwaves.admin` (зависит от режима) | `/kmobwaves highlight` или `/kmw highlight` |
